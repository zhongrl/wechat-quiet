...
public class DESUtil {
	private static Logger log = LoggerFactory.getLogger("DESUtil");
	...
	public String getURLEncode(String src) {
		...
		try {
			...
			log.error("对字符串进行URLDecoder.encode(strEncoding)编码异常", e);
		}
	...
	public String getURLDecoderdecode(String src) {
		...
		try {
			...
			log.error("对字符串进行URLDecoder.decode(strEncoding)解码异常", e);
		}
	...
	/**
	 * //会员可获取积分，1元1积分，取整(为了2月底积分上线，1元1积分)
	 * @param payMoney
	 * @return
	 */
	public static int getIntegral(double payMoney,int integralblil){
		//会员可获取积分，1元1积分，取整(为了2月底积分上线，1元1积分)
		int integral = (int)Math.floor(payMoney*integralblil);
		return integral;
	}
